...
def yio():
    global MAG, Yi, Yo, Gt,yif,yrf,yff,yof
    y=yrf*yff
    sq=((2*yif.real*yof.real-(y.real)))**2-((abs(y))**2)
    #print sq
    N=mt.sqrt(sq)
    Gi=N/(2*yof.real)
    Go=N/(2*yif.real)
    Bo=yof.imag-y.imag/(2*yif.real)
    Bi=yif.imag-y.imag/(2*yof.real)
    Yi=Gi+Bi*1j
    Yo=Go+Bo*1j
    MAG=((abs(yff))**2)/(4*yif.real*yof.real)
    print "\nLa MAG es= ", MAG
    print "\nLas impedancias de entrada y salida son:"
    print "\nYi= ", Yi
    print "\nYo= ", Yo
    Gt=((abs(yff))**2)/(((2*yif.real*yof.real-(y.real)))+N)
